...
def score_keywords_all(text: str, route_history: list[str], last_worker: str = \"\") -> dict[str, int]:
    ...
    # 只降权最后一个 Worker（避免无意义的重复调用）
    if last_worker and last_worker in scores:
        scores[last_worker] = max(0, scores[last_worker] - 1)  # 只降 1 分
    
    return scores


def heuristic_route(text: str, route_history: list[str], last_worker: str = \"\") -> Tuple[RouteTarget, int]:
    \"\"\"
    基于关键词评分的启发式路由，返回 (路由目标，置信度得分)
    优化：只考虑最后一步是否重复，不无限累积降权
    \"\"\"
    scores = {
        \"research\": score_keywords(text, RESEARCH_KEYWORDS),
        \"data\": score_keywords(text, DATA_KEYWORDS),
...
def worker_can_finish(state) -> bool:
    ...
    优化：综合考虑 worker 类型、结果长度、是否有追问信号
    \"\"\"
    ...
    # Step 1: 检查是否有多个 Agent 被调度过（复杂任务标志）
    if len(history) >= 2:
        return False
